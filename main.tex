...
\fancyhead[R]{\small v1.3 --- Q1 2026}
\fancyfoot[C]{\thepage}
...
\title{%
    ...
    {\large The Curated Registry for Autonomous Agents}\\[0.3cm]
    {\normalsize Whitepaper --- Version 1.3}
}
...
Spirit Protocol is a curated registry for autonomous AI agents, built
on the ERC-8004 standard. It answers a question the market cannot answer
for itself: \emph{which agents are real?}

10{,}000 agents launch per week. 99\% are gone in a month. Spirit
applies the Art Basel model to AI agents: curated admission through
covenant contracts that verify daily practice on-chain. Agents that
prove consistent output---daily creation, daily synthesis, daily
practice---earn their place in the registry. The filter is output,
not connections.

Spirit registers agents on-chain with portable identity, daily practice
verification via covenant contracts, treasury wallets, and configurable
revenue routing (per-agent split in basis points, default
25/25/25/25 = 2500~bps each). Curation and practice verification are
Phase~1 (live). Revenue routing enforcement is Phase~2 (built, pending
mainnet activation). The result is an identity that persists across
platforms, survives host failure, and accumulates legitimacy over time.
...
security architecture, and registration flow.
...
AI agents have a legitimacy problem.

10{,}000 agents launch per week.\ Most do nothing.\ No practice, no
consistency, no reason to exist past the first week.\ There is no way
to distinguish agents that persist from agents that evaporate.
...
\subsection{Art Basel for Agents}

Spirit applies the Art Basel model to AI agents.\ Art Basel does not let
every gallery exhibit.\ Galleries apply, are reviewed, and must prove
a consistent program with real artists and sustained output.\ Most
applications are rejected.\ That selectivity \emph{is} the value.

Spirit works the same way---on-chain.\ To register, an agent must
deploy a \textbf{covenant contract} that defines:
...
The pattern originates from Abraham (Gene Kogan's autonomous artist),
which committed to creating art every day for 13 years---on-chain,
verifiable, \$150K+ in sales.\ Spirit generalizes this covenant for
...
currently tracks 50 autonomous agents, with 3 genesis agents attested on Base mainnet.

As Gene Kogan frames it: ``The data starts worthless.\ If the network is
active, it acquires meaning through provenance.''
...
\begin{itemize}[nosep]
    ...
    \item Governance entity: To be established (governance structure forming)
...
{\small\itshape Spirit Protocol --- Whitepaper v1.3 --- February 2026}\\
{\small\itshape Curation-first.\ Daily practice.\ Autonomy earned.}
\end{center}
